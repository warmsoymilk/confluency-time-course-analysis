...
def format_column_name(s):
    if s.lower() == 'confluency':
        return 'Confluency'
    elif s.startswith('Fluorescence') and s.endswith('ratio'):
        return f'Fluorescence Ratio (Ex={s.split("_")[1]}nm)'
    else:
        return s.capitalize()
...
# Generate list of valid columns from saved data
def generate_valid_column_list():
    cols = []
    all_cols = set()
    ...
    valid_cols = [c for c in all_cols if (c == 'confluency' or (c.startswith('Fluorescence') and c.endswith('ratio'))) and all(c in colset for colset in cols)]
    valid_cols.sort(key=lambda s: s.capitalize())
    return valid_cols

# Generates a dialog to allow user to select data column for analysis
def generate_column_selection_dialog():
    valid_cols = generate_valid_column_list()
    return st.selectbox(
        'Select the data type to use for analysis',
        options=valid_cols,
        format_func=format_column_name,
        disabled=len(valid_cols) == 1,
    )
...
def edit_well_mapping():
    ...
    # Determine valid columns for analysis and show a dialog to allow choice
    selected_col = generate_column_selection_dialog()

    # Show dialog to allow user to edit well mapping
    st.write('Additionally, please review the autogenerated cross-timestamp well mapping and make any necessary changes. (Double-click on a label to edit it.)')
...
def prepare_for_plotting_time_course():
    ...
    pivot_df = df.pivot(index='__label', columns='timepoint', values=st.session_state.selected_col)
...
def show_time_course_plot():
    ...
    ax.set_ylabel(format_column_name(st.session_state.selected_col))
    ...
    # Display the plot
    plt.tight_layout()
    st.write('Here is a bar chart showing a per-well time course:')
    st.pyplot(plt)
...
# Allows the user to choose a column of the single file to analyze
def choose_single_file_column():
    valid_cols = generate_valid_column_list()
    if len(valid_cols) == 1:
        st.session_state.selected_col = valid_cols[0]
        increment_step()
    else:
        selected_col = generate_column_selection_dialog()
        if st.button('Continue'):
...
def plot_single_file():
    ...
    well_values = {row['well']: row[st.session_state.selected_col] for _, row in df.iterrows()}

    # Create a matrix to hold the confluency values
    value_matrix = np.full((len(rows), len(cols)), -1.0)

    # Populate the confluency matrix
    for i, row in enumerate(rows):
        for j, col in enumerate(cols):
            well_name = f"{row}{col}"
            if well_name in well_values:
                value_matrix[i, j] = well_values[well_name]

    # Create a custom colormap from white to red
    max_color = 'red' if st.session_state.selected_col == 'confluency' else 'green'
    cmap = LinearSegmentedColormap.from_list('white_red', ['white', max_color], N=256)

    # Plot the grid
    fig, ax = plt.subplots(figsize=(len(cols), len(rows)))

    # Display the confluency matrix using the colormap
    vmin = 0 if st.session_state.selected_col == 'confluency' else 1
    vmax = 1 if st.session_state.selected_col == 'confluency' else 2
    cax = ax.imshow(value_matrix, cmap=cmap, vmin=vmin, vmax=vmax, aspect='auto')
    ...
    for i in range(len(rows)):
        for j in range(len(cols)):
            if value_matrix[i, j] != -1.0:
                if st.session_state.selected_col == 'confluency':
                    text_color = 'black' if value_matrix[i, j] < 0.5 else 'white'
                else:
                    text_color = 'black'
                ax.text(j, i, f"{value_matrix[i, j]:.2f}", ha='center', va='center', color=text_color)

    # Add a colorbar to show the gradient
    cbar = fig.colorbar(cax, ax=ax, orientation='vertical')
    cbar.set_label(format_column_name(st.session_state.selected_col), fontsize=8)
    cbar.ax.tick_params(labelsize=8)

    # Show the plot
    plt.tight_layout()
    st.write(f'Per-well data for the selected plate ({st.session_state.files["name"].iloc[0]}):')
    st.pyplot(plt)
...
st.set_page_config(page_title='Cephla Imaging Data Analysis')
st.title('Cephla Imaging Data Analysis')
